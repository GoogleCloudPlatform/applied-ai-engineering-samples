...
# Usage:
# bash deploy_saxml.sh NAMESPACE=<namespace> KSA_NAME=<ksa_name> GSA_EMAIL=<gsa_email> ARTIFACT_REGISTRY_PATH=<artifact_registry_path>

set -e

usage() {
    echo -e "\n\nError: $1"
    echo "Usage: bash deploy_saxml.sh NAMESPACE=<namespace> KSA_NAME=<ksa_name> GSA_EMAIL=<gsa_email> ARTIFACT_REGISTRY_PATH=<artifact_registry_path>"
    exit 2
}
